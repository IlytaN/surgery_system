...
class SecretaryAppointments extends CI_Controller
{
    ...
    public function newappointment()
    {
        // CK: handles all the backend for creating an appointment - dummy so that the AJAX request can be made in teh JS
          $App=$this->input->post('AppId');
          $Pat=$this->input->post('PatId');
          $Slot=$this->input->post('SlotId');
          $received = array('AppId' => $App ,
                            'PatId' => $Pat,
                            'SlotId'=> $Slot
                            );
        $return = array('status' => $received);
        $this->output->set_content_type("application/json")->set_output(json_encode($return));
        // send data to Appointments_model
        $this->load->model('Appointments_model');
        $this->Appointments_model->addAppointment($received);
